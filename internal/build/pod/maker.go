...
func (m *maker) podSpec(mld *api.ModuleLoaderData, containerImage string, pushImage bool) v1.PodSpec {
	...
	var kanikoImagePullSecret []v1.LocalObjectReference
	if secretName := os.Getenv("RELATED_IMAGE_BUILD_PULL_SECRET"); secretName != "" {
		kanikoImagePullSecret = []v1.LocalObjectReference{
			{
				Name: secretName,
			},
		}
	...
	containerArgs := m.containerArgs(buildConfig, mld, containerImage, pushImage)
	_, isCIEnvSet := os.LookupEnv("CI_ENV")
	if isCIEnvSet {
		// Add this flag to bypass Kind cluster issue in CI
		containerArgs = append(containerArgs, "--ignore-path=/product_uuid")
	}
	...
	return v1.PodSpec{
		...
		ImagePullSecrets: kanikoImagePullSecret,
