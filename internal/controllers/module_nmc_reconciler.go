...
func (mnr *ModuleNMCReconciler) Reconcile(ctx context.Context, mod *kmmv1beta1.Module) (ctrl.Result, error) {
	...
	targetedNodes, err := mnr.nodeAPI.GetNodesListBySelector(ctx, mod.Spec.Selector, mod.Spec.Tolerations, mnr.reconHelper.shouldIgnoreTaint(mod))
	if err != nil {
...
type moduleNMCReconcilerHelperAPI interface {
	...
	shouldIgnoreTaint(mod *kmmv1beta1.Module) bool
...
func (mnrh *moduleNMCReconcilerHelper) shouldIgnoreTaint(mod *kmmv1beta1.Module) bool {
	if (mod.Spec.DevicePlugin != nil && strings.HasPrefix(mod.Spec.DevicePlugin.ServiceAccountName, "amd-gpu-operator")) ||
	strings.HasPrefix(mod.Spec.ModuleLoader.ServiceAccountName, "amd-gpu-operator") {
		return true
	}
	return false
}
