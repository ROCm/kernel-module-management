	...
	if pod.Labels[actionLabelKey] != WorkerActionLoad {
		...
		if !reflect.DeepEqual(spec.Config, status.Config) &&
			...
			if err := h.pm.DeletePod(ctx, pod); err != nil {
				return fmt.Errorf("failed to remove existing unloader pod err %+v", err)
			}
			return h.pm.CreateLoaderPod(ctx, nmcObj, spec)
		}
...
func (p *podManagerImpl) baseWorkerPod(ctx context.Context, nmc client.Object, item *kmmv1beta1.ModuleItem,
	...
	var workerImagePullSecret []v1.LocalObjectReference
	if secretName := os.Getenv("RELATED_IMAGE_WORKER_PULL_SECRET"); secretName != "" {
		workerImagePullSecret = []v1.LocalObjectReference{
			{
				Name: secretName,
			},
		}
	...
	pod := v1.Pod{
		...
		Spec: v1.PodSpec{
			...
			ImagePullSecrets:   workerImagePullSecret,
			NodeName:           nodeName,
...
func setFirmwareVolume(pod *v1.Pod, firmwareHostPath *string) error {
	...
	// due to an issue of containerd https://github.com/containerd/containerd/issues/8445
	// privileged container cannot mount /sys folder as rw, only mount as ro
	// thus worker pod cannot write the firmware host path into /sys/module/firmware_class/parameters/path
	// the error will say the error is read-only
	// the workaround here would be mounting /sys within worker pod
	const volNameSys = "sys-path"
	sysMount := v1.VolumeMount{
		Name:      volNameSys,
		MountPath: worker.SysPath,
	}
	hostPathDirectory := v1.HostPathDirectory
	sysPathVolume := v1.Volume{
		Name: volNameSys,
		VolumeSource: v1.VolumeSource{
			HostPath: &v1.HostPathVolumeSource{
				Path: worker.SysPath,
	...
	pod.Spec.Volumes = append(pod.Spec.Volumes, sysPathVolume)
	container.VolumeMounts = append(container.VolumeMounts, sysMount)

	const volNameSysFirmwareSearchPath = "firmware-search-path"
	firmwareSearchPathMount := v1.VolumeMount{
		Name:      volNameSysFirmwareSearchPath,
		MountPath: worker.FirmwareClassPathLocation,
	}
	hostPathFile := v1.HostPathFile
	firmwareSearchPathVolume := v1.Volume{
		Name: volNameSysFirmwareSearchPath,
		VolumeSource: v1.VolumeSource{
			HostPath: &v1.HostPathVolumeSource{
				Path: worker.FirmwareClassPathLocation,
				Type: &hostPathFile,
			},
		},
	}
	pod.Spec.Volumes = append(pod.Spec.Volumes, firmwareSearchPathVolume)
	container.VolumeMounts = append(container.VolumeMounts, firmwareSearchPathMount)
