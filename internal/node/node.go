...
type Node interface {
	...
	GetNodesListBySelector(ctx context.Context, selector map[string]string, tolerations []v1.Toleration, ignoreTaint bool) ([]v1.Node, error)
...
func (n *node) GetNodesListBySelector(ctx context.Context, selector map[string]string, tolerations []v1.Toleration, ignoreTaint bool) ([]v1.Node, error) {
	...
	for _, node := range selectedNodes.Items {
		if n.IsNodeSchedulable(&node, tolerations) || ignoreTaint {
...
func (n *node) GetNumTargetedNodes(ctx context.Context, selector map[string]string, tolerations []v1.Toleration) (int, error) {
	targetedNode, err := n.GetNodesListBySelector(ctx, selector, tolerations, false)
	if err != nil {
