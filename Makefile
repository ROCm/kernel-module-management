...
.PHONY: docker-save
docker-save: 
	docker save $(IMG) | gzip > kernel-module-management-operator.tar.gz

.PHONY: docker-build-hub
docker-build-hub: ## Build docker image with the hub manager.
	docker build -t $(HUB_IMG) --build-arg TARGET=manager-hub --label HOURLY_TAG=$(VERSION) .
...
bundle-build: ## Build the bundle image.
	docker build -f bundle.Dockerfile -t $(BUNDLE_IMG) --label HOURLY_TAG=$(VERSION) .
...
signimage-build: ## Build docker image with the signer.
	docker build -f Dockerfile.signimage -t $(SIGNER_IMG) --label HOURLY_TAG=$(VERSION) .

.PHONY: signimage-push 
signimage-push: ## Push docker image with the signer.
	docker push $(SIGNER_IMG)

.PHONY: signimage-save
signimage-save:
	docker save $(SIGNER_IMG) | gzip > kernel-module-management-signimage.tar.gz

.PHONY: webhookimage-build
webhookimage-build: ## Build docker image for the webhook server.
	docker build -f Dockerfile.webhook -t $(WEBHOOK_IMG) --label HOURLY_TAG=$(VERSION) .

.PHONY: webhookimage-push
webhookimage-push: ## Push docker image for the webhook server.
	docker push $(WEBHOOK_IMG)

.PHONY: webhookimage-save
webhookimage-save: ## Push docker image for the webhook server.
	docker save $(WEBHOOK_IMG) | gzip > kernel-module-management-webhook-server.tar.gz

.PHONY: workerimage-build 
workerimage-build: ## Build docker image for worker app.
	docker build -f Dockerfile.worker -t $(WORKER_IMG) --label HOURLY_TAG=$(VERSION) .

.PHONY: workerimage-push 
workerimage-push: ## Push docker image for worker app.
	docker push $(WORKER_IMG)

.PHONY: workerimage-save
workerimage-save: ## Push docker image for the worker.
	docker save $(WORKER_IMG) | gzip > kernel-module-management-worker.tar.gz
